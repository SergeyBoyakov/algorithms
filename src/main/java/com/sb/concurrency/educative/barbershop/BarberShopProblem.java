package com.sb.concurrency.educative.barbershop;

import java.util.concurrent.Semaphore;
import java.util.concurrent.locks.ReentrantLock;

public class BarberShopProblem {

    final int CHAIRS = 3;
    final Semaphore waitForCustomerToEnter = new Semaphore(0);
    final Semaphore waitForBarberToGerReady = new Semaphore(0);
    final Semaphore waitFOrCustomerToLeave = new Semaphore(0);
    final Semaphore waitForBarberToCutHair = new Semaphore(0);
    int waitingCustomers = 0;
    ReentrantLock lock = new ReentrantLock();
    int hairCutsGiven = 0;

    void customerWalksIn() throws InterruptedException {
        lock.lock();
        if (waitingCustomers == CHAIRS) {
            System.out.println("Customer walks out, all chairs occupied.");
            lock.unlock();
            return;
        }

        waitingCustomers++;
        lock.unlock();

        // lent the barber know you are here, in case he is asleep
        waitForCustomerToEnter.release();

        // wait for the barber to come take you to the salon chair hen its your turn
        waitForBarberToGerReady.acquire();

        // This is where the customer gets teh haircut

        waitForBarberToCutHair.acquire();
        waitFOrCustomerToLeave.release();

        lock.lock();
        waitingCustomers--;
        lock.unlock();

    }

    void barber() throws InterruptedException {
        while (true) {
             waitForCustomerToEnter.acquire();
             waitForBarberToGerReady.release();
            hairCutsGiven++;
            System.out.println("Barber cutting hair . . ." + hairCutsGiven);
            Thread.sleep(50);
            waitForBarberToCutHair.release();

            waitFOrCustomerToLeave.acquire();
        }

    }

}
